map.fisheries.data = function(p, outdir,  FUN, yrs, variable='effort',probs=c(0,0.975),log.variable=F,offset,theta=75,pres=10) {
  ...
  for(i in 1:length(yrs)){
    ...
    # do the map
    try( map( xyz[[i]], xyz.coords="planar", cfa.regions=T, depthcontours=T, pts=NULL, annot=yrs[i], fn=outfn, loc=outloc, at=datarange,colorkey=NULL , col.regions=cols(length(datarange)+1), colpts=F, corners=p$corners, display=F,rez=c(pres,pres)))
  }
}

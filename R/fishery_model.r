...
fishery_model = function(  p=NULL, DS="plot", assessment_years=2000:p$year.assessment, plotresults=TRUE, tag="default", areas=c("cfanorth", "cfasouth", "cfa4x"), vname="", type="density", res=NULL, fn=NULL, aulabels=c("N-ENS","S-ENS","4X") , save.plot=TRUE,
... ) {
...
#     sb = bio.snowcrab::fishery_model( DS="data_aggregated_timeseries", p=p, assessment_years=p$yrs )
  ...
  if (DS=="logistic_parameters") {
    ...
    if (!exists("standata", out)) {
      ...
      out$standata$IOA = scale(out$standata$IOA )
    }
    ...
    if (!exists("rmu", out$standata)) out$standata$rmu = c(0.6, 0.8, 1.0)
    ...
    if (!exists("qmu", out$standata)) out$standata$qmu = attr( out$standata$IOA,  "scaled:center" )
    if (!exists("qsd", out$standata)) out$standata$qsd =  attr( out$standata$IOA,  "scaled:scale" )  
  ...
  if (DS=="stan_surplus_production") {
    return( "
      ...
      parameters {
        ...
        vector <lower=eps, upper=1.5> [U] r;
        vector <lower=eps, upper=2> [U] q;
        vector <lower=eps > [U] Ymu;  // Zscore offset
...
//        matrix[MN,U] rem;  // observed catch
        ...
        // copy parameters to a new variable (Y) with imputed missing values
        {
          ...
          for (j in 1:U) {
            for (i in 1:N) {
                Y[i,j] = log( IOA[i,j] + Ymu[j] )  ;  // translation of a zscore to internal scale (positive valued)
                if ( missing[i,j] == 1 ) {
                ii = ii+1;
                Y[i,j] = IOAmissing[ii];
              }
      ...
      model {
        ...
        Ymu ~ normal( qmu, qsd ) ; // i.e., offset to original scale

        q ~ normal( 1.0, 0.25 ) ; // i.e., scaling factor to original scale
        ...
        // spring surveys
        for (j in 1:2) {
           Y[1, j]  ~ normal( log( K[j] * fmax(  q[j] * bm[1,j]  - CAT[1,j]/K[j], eps) ), bosd[j] ) ;
          for (i in 2:(ty-1) ){
             Y[i, j] ~ normal( log( K[j] * fmax( q[j] * bm[i,j]  - CAT[i-1,j]/K[j], eps) ), bosd[j] ) ;
          }
        }

        for (i in 1:(ty-1) ){
           Y[i, 3] ~ normal( log( K[3] * fmax( q[3] * bm[i,3]  - CAT[i,3]/K[3], eps) ), bosd[3] ) ;
        }


        //  transition year (ty)
        for (j in 1:2) {
          Y[ty,j]  ~ normal( log( K[j] * fmax( q[j] * bm[ty,j]  - (CAT[ty-1,j] + CAT[ty,j]) / (K[j] * 2.0), eps) ), bosd[j] ) ; //NENS and SENS
        }
        Y[ty,3]  ~ normal( log( K[3] * fmax( q[3] * bm[ty,3]  - CAT[ty,3]/K[3], eps) ), bosd[3] ) ; //SENS

        // fall surveys
        for (j in 1:3) {
          for (i in (ty+1):N) {
            Y[i,j] ~ normal( log( K[j] * fmax(  q[j] *   bm[i,j] - CAT[i,j]/K[j], eps) ), bosd[j] ) ; //   fall surveys
          }
        }
        ...
        target += - Y ;  // required due to log transf above
        target += - log( bm );

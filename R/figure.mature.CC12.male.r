  ...
  figure.mature.CC12.male = function( p, outdir, all.areas=T ) {

    eps = 1e-6
    
    classes = paste( "ma", c(13:9), ".CC1to2.no", sep="")
    ...
    # last check to make sure data exists for all regions for the plots
     ...
     fn = file.path( outdir, paste( "ma.CC1to2",areas[i], sep="." ) )
     ...
     pl = xyplot( total~yr|vars, data=td, ub=td$ubound, lb=td$lbound,
        layout=c(1,n.panels), xlim=xlim, scales = list(y = "free"),
            main=paste("Mature male CC1 and CC2 snow crab --", regions[i]), xlab="Year",

  ...
  map.logbook.locations = function(p, basedir, newyear=T, map.method="lattice"  ) {
    ...
    x = x[bio.polygons::polygon_inside(x, region="isobath1000m"),]
    ...
    if (map.method=="lattice" ) {
      ...
      for (y in years) {
        ...
        print(fn)
        ecmei::ecmei_map( toplot, cfa.regions=T, depthcontours=T, annot=annot, fn=fn, loc=basedir, corners=p$corners )
      }

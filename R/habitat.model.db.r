  ...
  habitat.model.db = function( ip=NULL, DS=NULL, v=NULL, p=NULL, yr=NULL, debug=F ) {
    ...
    if (DS %in% c("large.male.auxillary.data", "large.male.auxillary.data.redo") ) {
      ...
      # add groundfish data
      gf = bio.snowcrab.external.db (p=p, DS="set.snowcrab.in.groundfish.survey", vname="R0.mass", year.current=p$current.assessment.year )  ## right now, fixed only to R0.mass ... TO DO make more variable

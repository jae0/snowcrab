  ...
  figure.mature.CC5.male = function( p, outdir, all.areas=T ) {

    eps = 1e-6
    
    classes = paste( "ma", c(13:9), ".CC5.no", sep="")
    ...
    fn = file.path( outdir, paste("ma.CC5", areas[i], sep=".") )
    Cairo( file=fn, type="pdf", bg="white", units="in", width=6, height=6 )

      setup.lattice.options()
      pl = xyplot( total~yr|vars, data=td, ub=td$ubound, lb=td$lbound,
        layout=c(1,n.panels), xlim=xlim, scales = list(y = "free"),
            main=paste("Mature male CC5 snow crab --", regions[i]), xlab="Year",

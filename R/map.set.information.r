...
map.set.information = function(p, outdir, variables,mapyears, plot.method="levelplot",interpolate.method='tps', theta=p$pres*25, idp=2,log.variable=T,add.zeros=T,minN=10  , probs=c(0.025, 0.975), offset) {
    ...
    if(missing(variables)){
      variables = stmdat::variable.list.expand("all.data")
...
#
      ...
      if (plot.method=="levelplot") {
       ...
       try( stm::stm_map( xyz, xyz.coords="planar", cfa.regions=T, depthcontours=T, pts=set_xyz[,c("plon","plat")], annot=y, fn=outfn, loc=outloc, at=datarange , col.regions=cols(length(datarange)+1), colpts=F, corners=p$corners, display=F,colorkey=ckey))
      }

  ...
  figure.immature.skipmoulter.male = function( p, outdir, all.areas=T ) {
    
    classes = paste( "mi", c(12:8), ".skip.moulter.no", sep="")
    ...
    for (i in c(1:n.regions)) {
     ...
     fn = file.path( outdir, paste( "imm.skip",areas[i], sep="." ) )
     Cairo( file=fn, type="pdf", bg="white",  units="in", width=16, height=20, dpi=144 )
      ...
      pl = xyplot( total~yr|vars, data=td, ub=td$ubound, lb=td$lbound,
        layout=c(1,n.panels), xlim=xlim, scales = list(y = "free"),
            main=paste("Immature male skip moulter snow crab --", regions[i]), xlab="Year",

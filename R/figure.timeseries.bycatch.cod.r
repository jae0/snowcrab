...
figure.timeseries.bycatch.cod = function( outdir, all.areas=T ) {
  ...
  xlim='NULL'
  xlim[1] = min(td$year, na.rm=T) #Mandate move to Maritimes Region and species other than snow crab began to be weighed 
  xlim[2] = max(td$year, na.rm=T)
  
  xlabels = seq(as.numeric(xlim[1]), as.numeric(xlim[2]), 1)
  xlim[1] = as.numeric(xlim[1]) - 0.5
  xlim[2] = as.numeric(xlim[2]) + 0.5
  ...
  pl = xyplot( mean~year|region, data=td, ub=td$ub, lb=td$lb,
               ...
               main="Cod Biomass", xlab=list("Year", cex=1), ylab=list("Geometric mean kg / km^2", cex=1),
               #cex.lab=cex.lab, 
               cex.axis=cex.axis, 
               cex.main = cex.main,

...
surplus.production.simple.laplacesdemon.setup = function(LD) {
  zeta = as.vector(mvtnorm::rmvnorm(1, rep(0,LD$S), Sigma, method="chol" ))
  ...
  if (is.null( LD$Model )) {
    LD$Model = function(parm, LD) {
      ...
      loglik[LD$pos$S] = dnorm( log(S), log(Spred), S_sd, TRUE );
      ...
      if (LD$Missing$nO > 0) {
        LD$O[LD$Missing$O ] = rlnorm( LD$Missing$nO, parm[LD$pos$Omissing], sd=LD$cv )
        LD$O[LD$Missing$O ] = .Internal(pmax(na.rm=TRUE, LD$O[LD$Missing$O ], LD$eps )) ;
        loglik[LD$pos$Omissing] = dnorm( parm[LD$pos$Omissing], LD$O[LD$Missing$O ], LD$cv*R[LD$Missing$O ] )
      }

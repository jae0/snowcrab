...
#Run bio.groundfish::(inst/scripts/temperature.r), need to update year within
  #system.file(package="emei", "scripts", "01.temperature.R") or run next line
...
#  system.file(package="emgis", "scripts", "01.indicators.r")  
#  system.file(package="emgis", "scripts", "02.biochem.r") 
#  system.file(package="emgis", "scripts", "02.condition.r") 
#  system.file(package="emgis", "scripts", "02.landings.r") 
#  system.file(package="emgis", "scripts", "02.metabolism.r") 
#  system.file(package="emgis", "scripts", "02.sizespectrum.r") 
#  system.file(package="emgis", "scripts", "02.speciesarea.r")
#  system.file(package="emgis", "scripts", "02.*.speciescomposition") 
...
p$emei_global_family = gaussian(link=log)
p$emei_local_modelengine = "twostep"

# 11 hrs with these settings
p$emei_twostep_space = "krige"
p$emei_gam_optimizer=c("outer", "bfgs") 
p$emei_distance_statsgrid = 2 # resolution (km) of data aggregation (i.e. generation of the ** statistics ** )
p$emei_distance_prediction = p$emei_distance_statsgrid * 0.75 # this is a half window km
p$emei_distance_scale = 50

p = bio.snowcrab::snowcrab.parameters( p=p, DS="emei", varname=p$selection$name  )

# o = snowcrab_emei(p=p, DS="emei_inputs" )  # create fields for 
emei( p=p, DATA='snowcrab_emei( p=p, DS="emei_inputs" )', runmode="stage2" ) # 30 min

p = make.list( list( yrs=p$yrs), Y=p )
parallel.run( snowcrab_emei, p=p, DS="predictions.redo" ) # warp predictions to other grids
snowcrab_emei( p=p, DS="emei.stats.redo" ) # warp stats to other grids
snowcrab_emei( p=p, DS="complete.redo" )
snowcrab_emei( p=p, DS="baseline.redo" )
snowcrab_emei( p=p, DS="map.all" )
...
# o = snowcrab_emei(p=p, DS="emei_inputs" )  # create fields for 
DATA='snowcrab_emei( p=p, DS="emei_inputs" )'

emei( p=p, DATA=DATA, tasks=c("initiate", "globalmodel") ) # 30 min
#   emei( p=p, tasks=c( "serial_debug" ) ) # serial mode
#   emei( p=p, tasks=c( "continue" ) )    
emei( p=p, tasks=c( "stage1" ) ) #  3 hrs 
emei( p=p, tasks=c( "stage2" ) ) #   1 hrs
emei( p=p, tasks=c( "save" ) )
...
### --------- prediction success:
set = snowcrab_emei(p=p, DS="input_data", voi=p$selection$name )

...
p$stm_local_modelengine = "twostep"

# 11 hrs with these settings
p$stm_twostep_space = "krige"
p$stm_gam_optimizer=c("outer", "bfgs") 
p$stm_distance_statsgrid = 2 # resolution (km) of data aggregation (i.e. generation of the ** statistics ** )
...
stm( p=p, DATA=DATA, tasks=c("initiate", "globalmodel") ) # 30 min
#   stm( p=p, tasks=c( "serial_debug" ) ) # serial mode
#   stm( p=p, tasks=c( "continue" ) )    
stm( p=p, tasks=c( "stage1" ) ) #  3 hrs 
stm( p=p, tasks=c( "stage2" ) ) #   1 hrs
stm( p=p, tasks=c( "save" ) )

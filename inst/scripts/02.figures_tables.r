...
#  Carapace condition from trawl data  >= 95mm CW  ... not kriged .. simple proportions
  ...
  # counts of stations in each area
    ...
    for (reg in c("cfaall", "cfanorth", "cfasouth","cfa4x"  ) ) {
      d = stmdat::polygon_inside(set[,c("lon","lat")], reg)
    ...
    N = stmdat::polygon_inside(setdata[,c("lon","lat")], "cfanorth")
    S = stmdat::polygon_inside(setdata[,c("lon","lat")], "cfasouth")
    X = stmdat::polygon_inside(setdata[,c("lon","lat")], "cfa4x")

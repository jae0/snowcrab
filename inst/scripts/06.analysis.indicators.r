  ...
  p = bio.snowcrab::load.environment( year.assessment=year.assessment )
    ...
    "ecmei", "bio.utilities", "parallel", "sorted.ordination", "ecmd") ) )

  # setwd( project.datadirectory("ecmd") )
...
# data <- readWorksheet(wb)
  ...
  indic = indicators.ts.db( db="ecmd.all.glue" )  # glue all time-series together
  # indic = indicators.ts.db( db="ecmd.all" ) # load the glued version
 ...
 # indic$data$seismic.3D = cumsum.jae(indic$data$seismic.3D)
  ...
  indic = indicators.ts.db( db="ecmd.all" )

  d = ecmd.subset ( indic, type="keyfactors" )
#  save( d, file="/home/adam/tmp/ordin.rdata", compress=TRUE )

  Y = pca.analyse.data(d, t0, t1, fname=file.path(project.datadirectory("ecmd"), "keyfactors" ) )
...
for (i in .keyfactors) {
  ...
  Y = pca.analyse.data( indic$data, .human, t0, t1, fname=file.path(project.datadirectory("ecmd"), "human") )

  # fishery landings
  .fishery = c(indic$landings.NS, indic$landings.totals.NS )
  Y = pca.analyse.data( indic$data, .fishery, t0, t1, fname=file.path(project.datadirectory("ecmd"), "fishery" ))

  # fishery landed value
  .fishery.value = c(indic$landedvalue.NS, indic$landedvalue.totals.NS )
  Y = pca.analyse.data( indic$data, .fishery.value, t0, t1, fname=file.path(project.datadirectory("ecmd"), "landedvalue" ))

  # fishery -- overall
  .fishery = c(indic$landedvalue.NS, indic$landedvalue.totals.NS, indic$landings.NS, indic$landings.totals.NS )
  Y = pca.analyse.data( indic$data, .fishery, t0=1970, t1, fname=file.path(project.datadirectory("ecmd"), "fishery.overall" ))

  # snowcrab
  .snowcrab = c(indic$snowcrab, "groundfish.stratified.mean.totwgt.snowcrab", "groundfish.stratified.mean.totno.snowcrab" )
  Y = pca.analyse.data(indic$data, .snowcrab, t0, t1, fname=file.path(project.datadirectory("ecmd"), "snowcrab" ))

  # climate
  .climate = c( indic$climate )
  Y = pca.analyse.data(indic$data, .climate, t0, t1, fname=file.path(project.datadirectory("ecmd"), "climate" ))

  # ecosystem
  .ecosystem = c( indic$ecosystem )
  Y = pca.analyse.data(indic$data, .ecosystem, t0, t1, fname=file.path(project.datadirectory("ecmd"), "ecosystem" ))

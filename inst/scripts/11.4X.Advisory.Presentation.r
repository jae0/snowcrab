...
if(map.logs) {
    require(PBSmapping)
    bioLibrary ( 'bio.spacetime','bio.utilities','bio.polygons' )
...
if(map.logs.cpue) {
   ...
   logs = logbook.db(DS='odbc.logbook',yrs=2003:p$year.assessment)
...
# if(map.logs.month) {
#     require(PBSmapping)
#     bioLibrary(c('bio.spacetime','bio.utilities','bio.polygons'))
#    logs = logs.fixed
#     logs = makePBS(logs,polygon=F)

...
p = bio.snowcrab::load.environment()
 ...
 # Carapace condition breakdown by year
  ...
  i = stmdat::polygon_inside(x=set[, pcoords], region=p$regions.to.model, planar=T, proj.type=p$internal.projection )
  xs = set[ i, ]
  xs$dummy = 1
...
p = bio.snowcrab::load.environment()
 ...
 x = move
  ...
  x$lon = x$lon0
  x$lat = x$lat0
  i = stmdat::polygon_inside( x, region="cfa22outer" )
...
p = bio.snowcrab::load.environment()
  ...
  ii = stmdat::polygon_inside(set, region="cfa4x")
  set = set[ ii, ]
...
nens = stmdat::polygon_inside( set, region="cfanorth" )

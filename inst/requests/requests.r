...
p = bio.snowcrab::load.environment()
 ...
 # Carapace condition breakdown by year
  ...
  i = emaf::polygon_inside(x=set[, pcoords], region=p$regions.to.model, planar=T, proj.type=p$internal.projection )
  xs = set[ i, ]
  xs$dummy = 1
...
p = bio.snowcrab::load.environment()
 ...
 x = move
  ...
  x$lon = x$lon0
  x$lat = x$lat0
  i = emaf::polygon_inside( x, region="cfa22outer" )
...
p = bio.snowcrab::load.environment()
  ...
  ii = emaf::polygon_inside(set, region="cfa4x")
  set = set[ ii, ]
...
nens = emaf::polygon_inside( set, region="cfanorth" )

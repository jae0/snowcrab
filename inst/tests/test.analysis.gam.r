...
Test:     Biomass estimation via GAM / LME
  ...
  p = bio.snowcrab::load.environment()
    ...
    g = g[ , c("Y", "yr", "cfa", "plon", "plat", "t", "tamp", "z", "log.substrate.grainsize", "dZ", "ddZ"  ) ]
    ...
    Q = gamm(
          formula = Y ~  yr + s(plon,plat) +  s(t)  + s( tamp) +  s( z) +  s( dZ) +s(ddZ),
          correlation=corSpher(value=c( Vrange, Vpsill ), form=~plon+plat|yr, nugget=T ),
          data=g, family=Gamma()
    )

    Ql = gamm(
          formula = Y ~  s(yr) + s(plon,plat)  + s( tamp) + s(t)+ s( z) + s( dZ),
          correlation=corSpher(value=c( Vrange, Vpsill ), form=~plon+plat|yr, nugget=T ),
...
R = gls( Y ~  yr * cfa + ( bs( t )  + bs( tamp)  ) + bs( z) + bs( log.substrate.grainsize) + bs( ddZ) +bs( dZ),
...
hab = c( "log.substrate.grainsize" , "dZ", "ddZ" , "tmean" , "tamp",  "tsd.H", "t" , "tamplitude.climatology", "tsd" , "t" , "tsd", "z"  )
